...
# Function to save task data to a file
def save_task_data():
    with open("task_data.json", "w") as file:
        json.dump(task_data, file)

# Function to load task data from a file
def load_task_data():
    global task_data
    try:
        ...
        task_data = {}  # Initialize an empty dictionary if the file does not exist
...
async def show_groups_ids(update: Update, context: CallbackContext):
...
async def set_sources(update: Update, context: CallbackContext):
    ...
    save_task_data()  # Save task data after updating
...
async def set_targets(update: Update, context: CallbackContext):
    task_label = context.args[0]
...
async def main():
    load_task_data()  # Load task data at the start
